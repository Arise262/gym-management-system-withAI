...
function scorePayment(s: RetentionSignals): { n: number; detail: string } {
  ...
  // the ₱600 student rate, so ₱3,600 saturates the factor (a non-student's
  // ₱700 saturates a little sooner, which is the right direction anyway).
  // Scaled to what CBG actually charges — the old ₱5,000 ceiling came from an
  // invented ₱12,000 membership and left realistic arrears barely registering.
