...
/**
 * Tells the admin how the temporary password reached them. Normally
 * it was emailed to the person and the admin never sees it; if the email could not go out,
 * this is the one time it is shown — it is not stored anywhere readable.
 */
export function LoginNotice({ login, who = "trainer" }: { login: LoginDelivery; who?: "trainer" | "member" }) {
  ...
  if (login.emailed) {
    return (
      <Alert>
        ...
        <AlertDescription>
          A temporary password was sent to {login.email}. The {who} should change it after signing in.
        </AlertDescription>
      </Alert>
    );
  }
  ...
  return (
    <Alert className="border-amber-500/60">
      <AlertTitle>{login.reason} Give the {who} these details in person:</AlertTitle>
